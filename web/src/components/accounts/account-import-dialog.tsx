...
import { createAccounts, type Account } from "../../lib/server-api";
...
function MethodCard({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="w-full rounded-2xl border border-gray-200 dark:border-gray-700 bg-white p-0 text-left transition hover:border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:bg-gray-900"
    >
      <Card className="rounded-2xl border-0 bg-transparent shadow-none">
        <CardContent className="flex items-start gap-4 p-4">
          ...
          <div className="space-y-1">
            ...
            <div className="text-sm leading-6 text-gray-500 dark:text-gray-400">{description}</div>
          </div>
        </CardContent>
      </Card>
    </button>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <button
              ...
              className="inline-flex items-center gap-1 text-sm text-gray-500 dark:text-gray-400 transition hover:text-gray-800 dark:text-gray-200"
            >
              <ArrowLeft className="size-4" />
              返回导入方式
            </button>
            <span className="text-xs text-gray-400 dark:text-gray-500">当前识别 {tokenCount} 个 Token</span>
          </div>
          <div className="space-y-2">
            <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Access Token 列表</label>
            <Textarea
              placeholder="每行一个 Access Token..."
              value={tokenInput}
              onChange={(event) => setTokenInput(event.target.value)}
              className="min-h-56 resize-none rounded-xl border-gray-200 dark:border-gray-700"
            />
          </div>
          <div className="rounded-2xl border border-dashed border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="space-y-1">
                <div className="text-sm font-medium text-gray-800 dark:text-gray-200">从 TXT 文件导入</div>
                <div className="text-sm leading-6 text-gray-500 dark:text-gray-400">支持 `.txt`，文件内容也是一行一个 Token。</div>
              </div>
              <Button
                type="button"
                variant="outline"
                className="rounded-xl border-gray-200 dark:border-gray-700 bg-white"
    ...
    if (method === "session") {
      return (
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 p-4 text-sm leading-6 text-gray-600 dark:text-gray-400">
            打开
            {" "}
            <a
              href={sessionUrl}
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-1 font-medium text-gray-900 dark:text-gray-100 underline underline-offset-4"
            >
              {sessionUrl}
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Session JSON</label>
            <Textarea
              placeholder='粘贴完整 JSON，例如包含 "accessToken" 的对象...'
              value={sessionInput}
              onChange={(event) => setSessionInput(event.target.value)}
              className="min-h-56 resize-none rounded-xl border-gray-200 dark:border-gray-700 font-mono text-xs"
            />
          </div>
        </div>
      );
    ...
    if (method === "cpa") {
      return (
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-dashed border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 p-5">
            <div className="space-y-2">
              <div className="text-sm font-medium text-gray-800 dark:text-gray-200">多选本地 CPA JSON 文件</div>
            ...
            <Button
              type="button"
              className="mt-4 rounded-xl bg-gray-900 dark:bg-gray-100 dark:text-gray-900 text-white hover:bg-gray-800 dark:hover:bg-gray-200"
          ...
          />
          {pendingCpaImport ? (
            <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white p-4 text-sm leading-6 text-gray-600 dark:text-gray-400">
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <Button
          className="h-10 rounded-xl bg-gray-900 dark:bg-gray-100 dark:text-gray-900 px-4 text-white hover:bg-gray-800 dark:hover:bg-gray-200"
        ...
        <DialogContent showCloseButton={false} className="rounded-2xl p-6">
          ...
          <DialogFooter className="pt-2">
            ...
            {method === "cpa" ? (
              <Button
                className={cn(
                  "h-10 rounded-xl bg-gray-900 dark:bg-gray-100 dark:text-gray-900 px-5 text-white hover:bg-gray-800 dark:hover:bg-gray-200",
